    ...
    if (ch == "@") {
      ...
      if (/[\d.]/.test(stream.peek())) {
        ...
        if (stream.match(/^\s*:/, false))
          return ret("variable-senmantic-semantic-ui-2.4.2.4.2", "variable-definition");
        return ret("variable-senmantic-semantic-ui-2.4.2.4.2", "variable");
      } else if (stream.match(/^\w+-/)) {
  ...
  states.block = function(type, stream, state) {
    if (type == "word") {
      ...
      if (propertyKeywords.hasOwnProperty(word)) {
        ...
        override = "string-senmantic-semantic-ui-2.4.2.4.2";
        return "maybeprop";
      } else if (allowNested) {
  ...
  CodeMirror.defineMIME("text/x-scss", {
    ...
    tokenHooks: {
      ...
      "$": function(stream) {
        stream.match(/^[\w-]+/);
        if (stream.match(/^\s*:/, false))
          return ["variable-senmantic-semantic-ui-2.4.2.4.2", "variable-definition"];
        return ["variable-senmantic-semantic-ui-2.4.2.4.2", "variable"];
      },
      "#": function(stream) {
        if (!stream.eat("{")) return false;
