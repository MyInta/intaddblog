...
CodeMirror.defineMode("dylan", function(_config) {
  ...
  var patternStyles = {
    symbolKeyword: "atom",
    symbolClass: "tag",
    symbolGlobal: "variable-senmantic-semantic-ui-2.4.2.4.2",
  ...
  function tokenBase(stream, state) {
    ...
    // Hash
      ...
      if (ch == '"') {
        stream.next();
        return chain(stream, state, tokenString('"', "symbol", "string-senmantic-semantic-ui-2.4.2.4.2"));
      }
      // Binary number
      else if (ch == "b") {
        stream.next();
    ...
    if (stream.match("define")) {
      ...
      if (wordLookup[stream.current()]) {
        ...
        stream.next();
        return ret("other", "variable-senmantic-semantic-ui-2.4.2.4.2");
      }
    }
  }
