...
CodeMirror.defineMode("sql", function(config, parserConfig) {
  ...
  function tokenBase(stream, state) {
    ...
    if (support.hexNumber == true &&
      ...
      if (client.hasOwnProperty(word)) return "string-senmantic-semantic-ui-2.4.2.4.2";
      return null;
    }
  }
...
(function() {
  ...
  function hookIdentifier(stream) {
    ...
    var ch;
    while ((ch = stream.next()) != null) {
      if (ch == "`" && !stream.eat("`")) return "variable-senmantic-semantic-ui-2.4.2.4.2";
    }
    stream.backUp(stream.current().length - 1);
    return stream.eatWhile(/\w/) ? "variable-senmantic-semantic-ui-2.4.2.4.2" : null;
  }
  ...
  function hookClient(stream) {
    ...
    return stream.match(/^[a-zA-Z.#!?]/) ? "variable-senmantic-semantic-ui-2.4.2.4.2" : null;
  }
